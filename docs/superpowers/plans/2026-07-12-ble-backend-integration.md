...
- **C4. Proactive BLE switch-back**: ~~a BLE-state listener that re-selects BLE when it returns while pinned to Pi~~ — **SUPERSEDED 2026-07-19 by the passive-entry project.** Passive entry requires a PERSISTENT background BLE presence (`docs/superpowers/research/phone-key-authorization-and-passive-entry.md` §1.3/§3.1), which makes "BLE is up whenever you're near the car" the steady state and removes the stranded-on-Pi gap C4 existed to close. Building C4 first would also put a SECOND component in charge of the BLE link lifecycle, competing with the passive-entry connection manager — the same ownership-conflict class that caused the Pi orphan-session outage. BLE also stops being "one of two on-demand transports a selector picks" and becomes a persistent service with Pi as remote fallback, so C4's design targets a model that is being replaced. Do not implement; fold the need into passive entry.
