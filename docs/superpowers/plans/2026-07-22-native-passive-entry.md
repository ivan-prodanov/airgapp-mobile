...
**Tasks 4.1–4.4 DONE + deployed (2026-07-22); 4.5 on-car validation next.** Byte-pipe native surface (4.1, built into binary), `BridgedBleTransport` (4.2, 649 tests green), production cutover off ble-plx (4.3), auto-arm + fg/bg gate wiring (4.4, deploy-js). Smoke-verified at launch: AppDelegate defers in fg, restoration releases the one central to JS (no contention), JS arming effect auto-started native + set `foregroundResponderActive=true`, native connected in PIPE mode (no self-handshake) — model (b) exactly, **auto-armed with zero button taps**.
