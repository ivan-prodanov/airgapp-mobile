...
> - **Task 7 BUILT + installed (awaiting on-car background test):** `CBCentralManager` (restore id) is now created at APP LAUNCH via `PassiveEntryAppDelegate` (ExpoAppDelegateSubscriber, registered in the generated provider), so iOS can relaunch the suspended app and deliver `willRestoreState`. VIN persisted in UserDefaults at `start()` → background relaunch resumes with zero JS. Single-writer enforced natively: launch-time resume + `willRestoreState` only DRIVE the car when `applicationState==.background`; foreground releases the link to the JS path. Central built only once `isArmed()` (no launch-time BLE prompt on fresh installs). Smoke-tested: unarmed launch is a clean no-op.
