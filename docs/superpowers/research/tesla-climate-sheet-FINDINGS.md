...
- ~~**Home's outgoing card uses `forHorizontalIOS`** … a **118 px leftward parallax slide** … **This is the most likely thing you're missing**.~~

  > 🛑 **WRONG — corrected by Round 10 (`tesla-transitions-markers-FINDINGS.md`). Do NOT implement the parallax.** This reasoned from Home's *own* options, but CardStack's option merge (iOS 3332845-3332884) never consults them for a non-top card: `optionsForTransitionConfig = (index !== self.length-1 && nextOptions && nextOptions.presentation !== 'transparentModal') ? nextDescriptor.options : descriptor.options`. So when Controls is pushed, **Home's card takes Controls' `forFade`** — and `forFade` reads only `current` (never `next`), so Home's card sits at `opacity = current.progress = 1`, **completely static**. It neither slides nor fades. `forHorizontalIOS` is the preset default but is **unreachable on the Home↔Controls/Climate path**. What actually moves is Home's **content**, via its own `Animated.Value` (`duration = isFocused ? 300 : 0`) — see R10 §1c.
