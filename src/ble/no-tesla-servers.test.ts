...
const FORBIDDEN_LITERALS = ['tesla.com', 'owner-api', 'owners-api', 'akamai'];
...
test('src/ble contains no Tesla-server literals (tesla.com, owner-api, owners-api, akamai)', () => {
  const files = listFilesRecursive(BLE_DIR).filter((f) => {
    if (path.basename(f) === THIS_FILE) return false;
