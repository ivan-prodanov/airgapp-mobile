...
export type IvVariant = 'counter-last' | 'counter-first' | 'counter-last-le' | 'counter-first-le';

export const IV_VARIANTS: readonly IvVariant[] = [
  'counter-last',
  'counter-first',
  'counter-last-le',
  'counter-first-le',
...
export function buildAuthIv(counter: number, variant: IvVariant = 'counter-last'): Uint8Array {
  const iv = new Uint8Array(12);
  const c = counter >>> 0;
  const be = [(c >>> 24) & 0xff, (c >>> 16) & 0xff, (c >>> 8) & 0xff, c & 0xff];
  const le = [...be].reverse();
  const bytes = variant.endsWith('-le') ? le : be;
  if (variant.startsWith('counter-first')) iv.set(bytes, 0);
  else iv.set(bytes, 8);
  return iv;
}
